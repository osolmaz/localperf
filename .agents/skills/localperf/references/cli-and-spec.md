...
- one of `num_prompts` or `prompts_per_user`;
- positive `repeats`;
- a nonempty, unique `max_concurrency` list;
- a supported load generator and dataset.

For benchmark workloads, `prompts_per_user` must be at least 2 and every point
must resolve to at least `max(8, 2 * concurrency)` prompts. Diagnostics may use
smaller counts but are excluded from report and comparison queries.
